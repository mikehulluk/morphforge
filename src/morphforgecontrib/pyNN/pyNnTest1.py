...
spike_sourceE = create(SpikeSourceArray, {'spike_times': [float(i)
                       for i in range(1, 100, 1)]})
spike_sourceI = create(SpikeSourceArray, {'spike_times': [float(i)
                       for i in range(100, 200, 11)]})

connE = connect(spike_sourceE, hhcell, weight=0.02,
                synapse_type='excitatory', delay=2.0)
connI = connect(spike_sourceI, hhcell, weight=0.01,
                synapse_type='inhibitory', delay=4.0)
